...
class Enemies:
    ...
    def collision_box(self):
        # Mengembalikan collision box berdasarkan tipe musuh
        if self.type == 1:
            return pygame.Rect(
                (self.x, self.y),
                (Display.enemy_spaceship.get_width(), Display.enemy_spaceship.get_height())
            )
        else:  # type 2
            return pygame.Rect(
                (self.x, self.y),
                (Display.enemy_missile_craft.get_width(), Display.enemy_missile_craft.get_height())
            )
    ...
    def firing(self, player_x, player_y):
        # Only fire if the enemy is alive and on screen
        if not self.alive or self.x < 0 or self.x > Display.widthScreen:
            return

        # Random chance check for firing
        if random.random() > self.firing_chance:
            return

        # Enemy Type 1 (Spaceship) - Laser firing
        if self.type == 1 and len(self.bullets_list) < 2:
            # Calculate muzzle position (left center of the enemy spaceship)
            muzzle_x = self.x
            muzzle_y = self.y + (Display.enemy_spaceship.get_height() // 2)
            ...
            self.bullets_list.append({
                'x': muzzle_x,
                'y': muzzle_y - (Display.enemy_laser.get_height() // 2),  # Center the laser vertically
                'vel_x': trajectory_x,
                'vel_y': trajectory_y
            })
            Display.enemy_shoot_sound.play()

        # Enemy Type 2 (Missile Craft) - Missile firing
        elif self.type == 2 and len(self.missiles_list) < 3:
            # Calculate missile launch position (left center of the missile craft)
            launch_x = self.x
            launch_y = self.y + (Display.enemy_missile_craft.get_height() // 2)
            ...
            self.missiles_list.append({
                'x': launch_x,
                'y': launch_y - (Display.missile.get_height() // 2),  # Center the missile vertically
                'vel_x': trajectory_x,
                'vel_y': trajectory_y
            })
...
class Objects:
    def __init__(self):
        ...
        self.enemy_count = random.randint(8, 10)
