...
class Objects:
    ...
    def laser_hit(self, rect: pygame.Rect, laser: list):
        # Deteksi tabrakan dengan pesawat musuh
        for enemy in self.enemies_list:
            collision = rect.colliderect(enemy.collision_box())
            if collision and enemy.alive:
        ...
        # Deteksi tabrakan dengan proyektil musuh
        for enemy in self.enemies_list:
            for projectile in list(enemy.bullets_list):
                if enemy.type == 1:
                    proj_rect = pygame.Rect(
                        projectile[0] - Display().enemy_spaceship.get_width(),
                        projectile[1] + 28,
                        Display().enemy_laser.get_width(),
                        Display().enemy_laser.get_height()
                    )
                else:
                    proj_rect = pygame.Rect(
                        projectile[0] - 80,
                        projectile[1],
                        Display().missile.get_width(),
                        Display().missile.get_height()
                    )

                if rect.colliderect(proj_rect):
                    Display().explosion_sound.play()
                    if laser in self.laser_list:
                        self.laser_list.remove(laser)
                    enemy.bullets_list.remove(projectile)
                    self.score += 1
                    break
