...
backgrounds = [
    ...
    # Tambahkan latar belakang lain jika tersedia
]
...
class PlayerShip(pygame.sprite.Sprite):
    ...
    def shoot(self):
        ...
        if current_time - self.last_shoot_time >= self.cooldown:
            bullet = Bullet(self.rect.right, self.rect.centery, "right")
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, image, speed, shoot_interval):
        super().__init__()
        self.image = image
        self.rect = self.image.get_rect()
        self.speed = speed
    ...
    def shoot(self):
        ...
        bullet = Bullet(self.rect.left, self.rect.centery, direction)
...
class ZigzagEnemy(Enemy):
    def __init__(self, image, speed, shoot_interval):
        super().__init__(image, speed, shoot_interval)
        self.direction = 1
        self.zigzag_timer = 0
        self.zigzag_speed = 2
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, direction):
        ...
        if direction == "right":
            ...
            self.velocity = pygame.math.Vector2(10, 0)
        else:
...
# Level
levels = [
    {"enemies": [{"type": "straight", "count": 10}]},
    {"enemies": [{"type": "straight", "count": 5}, {"type": "zigzag", "count": 5}]},
    {"enemies": [{"type": "zigzag", "count": 5}, {"type": "sinusoidal", "count": 5}]},
]
...
def start_level(level_index):
    ...
    for enemy_type in level_data["enemies"]:
        for _ in range(enemy_type["count"]):
            if enemy_type["type"] == "straight":
                enemies_to_spawn.append(StraightEnemy(enemy_spaceship, 3, 2000))
            elif enemy_type["type"] == "zigzag":
                enemies_to_spawn.append(ZigzagEnemy(enemy_spaceship, 3, 2000))
            elif enemy_type["type"] == "sinusoidal":
                enemies_to_spawn.append(SinusoidalEnemy(enemy_spaceship, 3, 2000))
...
while running:
    ...
    if game_state == "playing":
        ...
        # Periksa tabrakan
        for bullet in pygame.sprite.groupcollide(player_bullets, enemies, True, True).keys():
            score += 10
        ...
        # Gambar latar belakang (scrolling)
        background = backgrounds[0]
        background_x -= 2
        if background_x <= -widthScreen:
            background_x = 0
        screen.blit(background, (background_x, 0))
        screen.blit(background, (background_x + widthScreen, 0))
