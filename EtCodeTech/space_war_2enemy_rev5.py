...
SPACESHIP_SPEED = 50  # Reduced speed for normal movement
BULLET_SPEED = 300
FPS = 60
...
class Spaceship:
    def __init__(self, x, y, image, controls, is_ai=False):
        ...
        self.velocity_x = random.choice([-SPACESHIP_SPEED, SPACESHIP_SPEED]) if is_ai else 0
        self.velocity_y = random.choice([-SPACESHIP_SPEED, SPACESHIP_SPEED]) if is_ai else 0
        self.image = image
...
class Bullet:
    ...
    def draw(self):
        ...
        pygame.draw.circle(screen, RAINBOW_COLORS[self.color_index], (int(self.x), int(self.y)), BULLET_RADIUS)
        pygame.draw.circle(screen, WHITE, (int(self.x), int(self.y)), BULLET_RADIUS + 2, 1)
