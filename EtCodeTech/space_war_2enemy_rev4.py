...
SPACESHIP_SPEED = 100  # Adjusted for normal speed
BULLET_SPEED = 300
FPS = 60
...
class Spaceship:
    ...
    def move(self):
        if self.is_ai:
            ...
            if keys[self.controls['down']] and keys[self.controls['right']]:
                self.angle = 315
                self.velocity_x = SPACESHIP_SPEED / 1.4
                self.velocity_y = SPACESHIP_SPEED / 1.4

            if keys[self.controls['down']] and keys[self.controls['left']]:
                self.angle = 225
                self.velocity_x = -SPACESHIP_SPEED / 1.4
                self.velocity_y = SPACESHIP_SPEED / 1.4
...
class Bullet:
    ...
    def draw(self):
        if self.owner == spaceship2:
            ...
            pygame.draw.circle(screen, BLUE, (int(self.x), int(self.y)), BULLET_RADIUS + 2, 1)
