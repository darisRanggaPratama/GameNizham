...
# Background Window
background = pg.image.load("OOP/picture/background2.jpeg")
...
game_over_font = pg.font.Font(None, 74)  # Larger font for game over message
...
players = [
    Player(x=200, y=300, is_main=True, control_scheme='arrows'),
]
enemy_list = []
for i in range(random.randint(5, 10)):
    enemy_list.append(Enemy())
...
while runs:
    ...
    if not game_active:
        ...
        game_over_text = game_over_font.render(f"GAME OVER - {winner} WINS!", True, BLUE)
        ...
        if current_time - game_over_time >= 3:
            reset_game()
    ...
    if game_start_time is not None:
        game_time = int(current_time - game_start_time)
    ...
    if not players:  # All players are dead
        game_active = False
        winner = "ENEMY"
        game_over_time = current_time
        continue
    ...
    if score >= 100 and len(players) == 1:
        players.append(Player(x=100, y=300, is_main=True, control_scheme='wasd'))
    ...
    time_text = font.render(f"Time: {game_time}s", True, BLUE)
    screen.blit(time_text, (10, 50))

    # Draw control scheme reminder
    if len(players) == 2:
        controls_text = font.render("Tank 1: Arrows | Tank 2: WASD", True, BLUE)
